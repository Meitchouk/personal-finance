...
import type { Account, Category, Transaction, TransactionTemplate, TransactionType } from "@/lib/types";
import { getAccountIcon, ACCOUNT_TYPE_LABELS } from "@/components/accounts/AccountBadge";
import { toast } from "sonner";
import { format } from "date-fns";
import { ArrowRightLeft, Bookmark, TrendingDown, TrendingUp } from "lucide-react";
...
interface Props {
  ...
  onSuccess: (savedTemplate?: TransactionTemplate) => void;
  onCancel: () => void;
}
...
export default function TransactionForm({ categories, accounts = [], transaction, defaultValues, onSuccess, onCancel }: Props) {
  const [loading, setLoading] = useState(false);
  const [saveAsTemplate, setSaveAsTemplate] = useState(false);
  const [recurrenceMode, setRecurrenceMode] = useState<"auto" | "manual">("auto");
  const [rates, setRates] = useState(DEFAULT_EXCHANGE_RATES);
  const [form, setForm] = useState({
    amount: (transaction?.original_amount ?? transaction?.amount)?.toString() ?? defaultValues?.original_amount?.toString() ?? "",
    original_currency: (transaction?.original_currency ?? defaultValues?.original_currency ?? BASE_CURRENCY) as CurrencyCode,
    type: (transaction?.type ?? defaultValues?.type ?? "expense") as TransactionType,
    ...
    account_id: transaction?.account_id ?? defaultValues?.account_id ?? "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      let savedTemplate: TransactionTemplate | undefined;
      if (!transaction && saveAsTemplate) {
        ...
        if (tplRes.ok) {
          ...
          savedTemplate = data as TransactionTemplate;
          toast.success("Guardado como acceso rápido");
        }
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
      ...
      {accounts.length > 0 && (
        <div className="space-y-1.5">
          <Label>Cuenta / Origen <span className="text-xs font-normal text-muted-foreground">(opcional)</span></Label>
          <Select value={form.account_id} onValueChange={(v) => set("account_id", v ?? "")}>
            <SelectTrigger className="w-full">
              {selectedAccount ? (
                ...
                <span className="text-muted-foreground">Sin cuenta asignada</span>
              )}
            </SelectTrigger>
            <SelectContent>
              ...
              {accounts.map((a) => {
                ...
                return (
                  <SelectItem key={a.id} value={a.id}>
                    ...
                    <span className="ml-2 text-xs text-muted-foreground">{ACCOUNT_TYPE_LABELS[a.type]}</span>
                  </SelectItem>
                );
              })}
            </SelectContent>
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    setSaveAsTemplate(mode === "manual");
                  }}
                  className={cn(
                    "flex flex-col items-center rounded-lg px-2 py-2 text-xs font-medium transition-all",
                    active
                ...
                >
                  <span className="font-semibold">
                    {mode === "auto" ? "Automático" : "Acceso rápido"}
                  </span>
                  <span className={cn("mt-0.5 text-[10px] font-normal", active ? "text-muted-foreground" : "text-muted-foreground/60")}>
                    {mode === "auto"
                      ? "Lo registro con su fecha"
                      : "Confirmo yo cuándo ocurre"}
                  </span>
                </button>
              );
            })}
      ...
      {/* Save as quick-access template */}
      {!(form.is_recurring && recurrenceMode === "manual") && (
        <button
          type="button"
        ...
        >
          <Bookmark className={cn("h-4 w-4 shrink-0", saveAsTemplate && "fill-primary text-primary")} />
          <div className="min-w-0">
            <p className="text-sm font-medium leading-tight">Guardar como acceso rápido</p>
            <p className="text-xs text-muted-foreground">
              Aparecerá arriba para registrarla rápidamente otra vez
            </p>
          </div>
        </button>
      )}
      {form.is_recurring && recurrenceMode === "manual" && (
        <div className="flex items-center gap-3 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2.5">
          ...
          <div className="min-w-0">
            <p className="text-sm font-medium leading-tight">Se guardará como acceso rápido</p>
            <p className="text-xs text-muted-foreground">
              Aparecerá arriba para confirmarla cuando te toque
            </p>
          </div>
        </div>
      )}
