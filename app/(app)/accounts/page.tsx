...
import { ACCOUNT_TYPE_LABELS, getAccountIcon } from "@/components/accounts/AccountBadge";
...
export default function AccountsPage() {
  ...
  const [form, setForm] = useState<FormState>({ name: "", type: "bank", color: DEFAULT_COLORS.bank });
  const [saving, setSaving] = useState(false);

  function openCreate() {
    setEditing(null);
    setForm({ name: "", type: "bank", color: DEFAULT_COLORS.bank });
    setOpen(true);
  }
      ...
      <AppSheet open={open} onOpenChange={setOpen} title={editing ? "Editar cuenta" : "Nueva cuenta"}>
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 rounded-xl border border-border bg-muted/30 p-3">
              ...
              >
                <TypeIcon className="h-5 w-5 transition-colors" style={{ color: form.color }} />
              </span>
              <div>
                <p className={cn("text-sm font-medium", !form.name && "text-muted-foreground")}>
                  {form.name || "Nombre de la cuenta"}
                </p>
                <p className="text-xs text-muted-foreground">{ACCOUNT_TYPE_LABELS[form.type]}</p>
              </div>
            </div>
